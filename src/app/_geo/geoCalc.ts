...
export function generateMiles (trail: Trail, waypoints: Array<Waypoint>, pois: Array<Poi>, snow: Array<object>): Trail {
  ...
  // findNearestMileInTree(pois[0].waypoint, _tree);
...
function createMiles (waypoints: Array<object>, pois: Array<Poi>, snow: Array<object>, scale: number): Array<Mile> {
  ...
  for (let i = 0; i < _wayPointsLength; i++) {
    ...
    if (_bridgedDistance > Settings.MILE || waypoints.length - 1 === i) {
      ...
      _miles.push({
        ...
        isCurrent:        false,
        snowData:         snow[_miles.length] as Array<Snowpoint>
      });
...
// create data tree
function createMileTree(startData :Array<Waypoint>, returnArray: Array<Array<Waypoint>>): Array<Array<Waypoint>> {
  ...
  if (resultArray.length > 2) {
    createMileTree(resultArray, returnArray);
  } else {
    return returnArray;
  }
}

export function findNearestMileInTree(location: Waypoint):object {

  let nearestMile: object;

  for (let i = 0; i < _tree.length; i ++) {

    const orderedWaypoints: Array<object> = geolib.orderByDistance(location, _tree[i]);
...
function linkPoisToMiles(pois: Array<Poi>, miles: Array<Mile>): void {
  ...
  for (let poi of pois) {
    ...
    if (String(poi.type).includes('water')) {
      trailData.waterSources.push(poi);
...
export function anchorDistanceCalculation(location: Waypoint, nearestMile: Mile, nearestWaypoints: object) {
...
function simplify (points: Array<any>, tolerance: number, highestQuality: boolean = true) {

  const originalPointCount: number = points.length;
  const sqTolerance: number = tolerance !== undefined ? tolerance * tolerance : 1;

  points = highestQuality ? simplifyRadialDistance(points, sqTolerance) : points;
...
function simplifyRadialDistance(points, sqTolerance) {
  ...
  let prevPoint: object = castAsNumbers(points[0]);
  const newPoints: Array<object> = [prevPoint];
  let point: object;

  for (let i = 1, len = points.length; i < len; i++) {
    // cast as number
    point = castAsNumbers(points[i]);
...
function castAsNumbers(point: object) {
